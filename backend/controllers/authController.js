...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler("Please check your email and try again.", 404)
    );
  }
  ...
  const message = `Your passsword reset token is as follows:\n\n${resetUrl}\n\nIf you have not requested reset password for this email, then ignore it.`;
...
exports.getUserDetails = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(`User with id ${req.params.id} does not exit`, 404)
    );
  }
...
exports.deleteUser = catchAsyncErrors(async (req, res, next) => {
  ...
  const email = (user.email = await cloudinary.v2.uploader.destroy(
